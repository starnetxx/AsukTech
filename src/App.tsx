...
function AppContent() {
  ...
  if (authUser) {
    // Check if this is an admin route and user might be admin
    const isAdminRoute = window.location.pathname.includes('/admin');
    
    // If it's an admin route, wait a bit more for admin role detection
    if (isAdminRoute && !isAdmin && !loading) {
      // Show loading while we wait for admin role detection
      return (
        <div className="min-h-screen bg-[#4285F4] flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            <div className="w-12 h-12 border-4 border-white/30 border-t-white rounded-full animate-spin"></div>
